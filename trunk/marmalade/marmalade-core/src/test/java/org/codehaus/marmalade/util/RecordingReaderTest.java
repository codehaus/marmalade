...
public class RecordingReaderTest extends TestCase {
    ...
    public void testShouldStoreAndReadCorrectlyWhenRecording() throws IOException {
        ...
        assertEquals("Number of characters read but unrecorded should be equal to unrecorded test input.", unRec.length(), numRead1);
        assertEquals("Number of characters read and recorded should be equal to recorded test input.", rec.length(), numRead2);
        assertEquals("Result of read before recording should be equal to unrecorded test input.", unRec, String.valueOf(cbuf1));
        assertEquals("Result of read after recording should be equal to recorded test input.", rec, String.valueOf(cbuf2));
